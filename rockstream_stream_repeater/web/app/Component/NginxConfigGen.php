...
class NginxConfigGen
{
    ...
    {
        # nginx config folder
        $nginx_file = (((AppInterfaces::getsetting('IS_CUSTOM_NGINX_BINARY') == TRUE && !empty(AppInterfaces::getsetting('NGINX_BINARY_DIRECTORY'))) ? AppInterfaces::getsetting('NGINX_BINARY_DIRECTORY') : Utility::defaultBinDirFolder('nginx')) . '\conf');
        ...
        $base_nginx .= N;
        $base_nginx .=          T . T . 'on_publish ' . config('component.web_url_checker_rtmp_url') . ':' . config('component.web_url_checker_rtmp_port')  . '/api/stream/on_publish;' . N;
        $base_nginx .=          T . T . 'on_publish_done ' . config('component.web_url_checker_rtmp_url') . ':' . config('component.web_url_checker_rtmp_port')  . '/api/stream/on_publish_done;' . N;
        if (AppInterfaces::getsetting('USE_LIVE_PREVIEW') == TRUE) {
            ...
            $base_nginx .=          T . T . 'hls_fragment 3;' . N;
        ...
        $base_nginx .=      T . 'server {' . N;
        $base_nginx .=          T . T . 'listen ' . config('component.nginx_port_rtmp') . ';' . N;
        $base_nginx .=          T . T . 'location /status {' . N;
